...
ExecStart={project_path}/venv/bin/python manage.py runserver 0.0.0.0:{django_port}
Restart=on-failure
RestartSec=5s
...
def create_django_service():
    ...
    user = input("Usuario que correrá el servicio (default: www-data): ").strip() or "www-data"
    django_port = input("Puerto Django (default: 8001): ").strip() or "8001"
    project_path = str(PROJECT_ROOT)
...
"""
    ...
    try:
        ...
        subprocess.run(['systemctl', 'daemon-reload'], check=True)
        print_success(f"Servicio creado: {service_file}")
        
        if input("\n¿Habilitar servicio al iniciar? (s/n): ").lower() == 's':
            subprocess.run(['systemctl', 'enable', service_name], check=True)
            print_success(f"{service_name} habilitado al inicio")
        
        if input("¿Iniciar servicio ahora? (s/n): ").lower() == 's':
            subprocess.run(['systemctl', 'start', service_name], check=True)
            print_success(f"{service_name} iniciado")
...
def create_frontend_service():
    ...
    user = input("Usuario que correrá el servicio (default: www-data): ").strip() or "www-data"
    react_port = input("Puerto React (default: 3000): ").strip() or "3000"
    project_path = str(PROJECT_ROOT)
...
def main():
    if len(sys.argv) < 2:
        ...
        print("1. Crear servicio Django")
        print("2. Crear servicio Frontend (React)")
        print("3. Ver estado de servicios")
        print("4. Ver logs")
        print("5. Recargar Nginx")
